...
class ConvFeatNet(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5, x6):
        ...
        x = torch.cat([xx1, xx2[:, :, :xx1.shape[-1]], xx3[:, :, :xx1.shape[-1]], xx4[:, :, :xx1.shape[-1]], xx5[:, :, :xx1.shape[-1]]], 1)
...
class PointNetDet(nn.Module):
    ...
    def forward(self,
        ...
        feat1, feat2, feat3, feat4, feat5, feat6 = self.feat_net(
            object_point_cloud_xyz,
            [center_ref1, center_ref2, center_ref3, center_ref4, center_ref5, center_ref6],
            object_point_cloud_i,
            one_hot_vec)

        x = self.conv_net(feat1, feat2, feat3, feat4, feat5, feat6)
